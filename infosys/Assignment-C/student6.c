/**********************************************************
* File			: Exercise 6 (student-6.c)
* Description	: Usage of strings
* Author		: 025 (Group 8)
* Version		: 1.00
* Date			: 6rd May 2014
* Log			: - edit student-5.c to insert new code
*				  - added iaProg 2d array
*				  - replace cStu_name with iaName 2d array
***********************************************************/
...
int main (int argc, char **argv)
{
	...
	char iaProg[5][26] = {"C","CPP","Java","Oracle","DotNet"};	//for storing following courses
	char iaName[5][26];											//for storing student names
	/*char cStu_name[100];			**removed**					//for student name in 1D array */
	...
	***************/
	...
	//Print all 5 courses from array
	for (k=0;k<5;k++) {
		printf("Course %d = %s\n", k, iaProg[k]); 
	}
	...
	for (i=0; i<5; i++) {
			...
			if (iStu_id > 1000 && iStu_id <= 1005) {
				...
				if (iSems == 5) {
					...
					scanf(" %[^\n]", iaName[i]);					//for some reason gets() do not work
